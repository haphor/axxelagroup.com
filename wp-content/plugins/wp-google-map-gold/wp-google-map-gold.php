...
if ( ! class_exists( 'Google_Maps_Pro' ) ) {
	...
	class Google_Maps_Pro
	{
		...
		jQuery(document).ready(function($) {
	        ...
	        function get_exact_names(result, type){
			...
			$("#wpgmp_metabox_location_city").val(get_exact_names(metabox_place, 'administrative_area_level_3') || get_exact_names(metabox_place, 'locality'));
			$("#wpgmp_metabox_location_state").val(get_exact_names(metabox_place, "administrative_area_level_1"));
			$("#wpgmp_metabox_location_country").val(get_exact_names(metabox_place, "country"));
	...
	<?php
		...
		_e( "Do you want to assign a category? Please create category <a href='".esc_url ( admin_url( 'admin.php?page=wpgmp_form_group_map' ) )."' target='_blank'>Here</a>", WPGMP_TEXT_DOMAIN );
		echo '</p>';
	}
		?>
		</div>
	...
	<?php
		...
		function wpgmp_save_meta_box_data($post_id) {
			if ( isset( $_POST['wpgmp_hidden_flag'] ) ) {
				...
				$wpgmp_enter_city = $_POST['wpgmp_metabox_location_city'];
				$wpgmp_enter_state = $_POST['wpgmp_metabox_location_state'];
				$wpgmp_enter_country = $_POST['wpgmp_metabox_location_country'];
