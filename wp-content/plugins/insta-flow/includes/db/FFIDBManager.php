...
class FFIDBManager extends LADBManager{
    ...
    private function getLocationMeta($locationId){
		$conn = FFDB::conn();
	    $locationId = $conn->parse('?p', $locationId);
		if (false !== ($location = $conn->getOne("select distinct p.location FROM ?n p WHERE p.location != '' AND p.location LIKE '%\"id\":\"{$locationId}\"%'", $this->posts_table_name))){
