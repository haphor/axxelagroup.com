...
//		load_plugin_textdomain( $domain, FALSE, basename( plugin_dir_path( dirname( __FILE__ ) ) ) . '/languages/' );
        load_plugin_textdomain( $domain, FALSE, basename( plugin_dir_path( dirname( __DIR__ ) ) ) . '/languages/' );
	}
	 ...
	 * @param $post_id
	 */
	private function process4comments($post_id){
		$time = time();
		...
		if(count($comments) === 0 || ($comments[0]["updated_time"] < $expiration) ){
			$instagram = new FFInstagram();
			$instagram->init($this->context, $this->generalSettings, null);
			$comments = $instagram->getComments($post_id);
			
			// Save comments to DB
			if (sizeof($comments) > 0 && FFDB::beginTransaction()){
				$sql_delete = "DELETE FROM ?n WHERE `post_id` = ?s";
				FFDB::conn()->query($sql_delete, $this->db->comments_table_name, $post_id);
				foreach ( $comments as $comment ) {
					$sql_insert = "INSERT INTO ?n SET `id` = ?s, `post_id` = ?s,  `from` = ?s, `text` = ?s, `created_time` = ?s, `updated_time` = ?s ON DUPLICATE KEY UPDATE ?u";
					FFDB::conn()->query($sql_insert, $this->db->comments_table_name, $comment->id, $post_id, json_encode($comment->from), $comment->text, $comment->created_time, $time, array('updated_time' => $time));
				}
				FFDB::commit();
			}
		}
	...
	private function createFeedInstances($feeds) {
		...
		if (is_array($feeds)) {
			foreach ($feeds as $feed) {
				...
				$clazz = new \ReflectionClass( 'ffinst\\flow\\social\\FF' . ucfirst($feed->$wpt) );//don`t change this line
